...
import es.NTTEnterprise.RIntellix.ms_risk_engine.domain.entities.simulation.RiskFeature;
...
/**
 * Mapper class responsible for converting ScoringDTO objects into Scoring entities.
 ...
 * It encapsulates the logic for mapping the fields from the ScoringDTO to the Scoring entity,
...
public class ScoringMapper {
     ...
     * This method takes a ScoringDTO, which is a data transfer object typically used for
     * communication with external services, and converts it into a Scoring entity that is used
     * within the domain layer of the application. The mapping includes transferring basic fields,
     * as well as converting the risk metrics, explainability features, and input snapshot into
     * their corresponding domain representations.
     * @param scoringDTO The ScoringDTO object to be mapped, containing the data retrieved from an external source.
    ...
    public Scoring toDomain(ScoringDTO scoringDTO) {
        ...
        scoring.setInputSnapshot(mapInputSnapshot(scoringDTO));
     ...
     * This method extracts the relevant risk metrics (PD, LGD, EAD, ECL) from the ScoringDTO.
    ...
    /**
     * Maps the explainability features from the list of TopFeatureDTO to a list of RiskFeature entities.
     * This method converts each TopFeatureDTO, which contains the feature name, value and SHAP value,
     * into a RiskFeature entity that can be used in the domain layer.
     * @param featureImportanceDTOs The list of TopFeatureDTO objects representing the
     *                              top contributing features and their SHAP values.
     * @return A list of RiskFeature entities mapped from the provided TopFeatureDTOs. If the input list is null, an empty list is returned.
     *
     */
     ...
     * This method takes the input features provided in the ScoringDTO and converts them into a
     * ModelInputs entity that encapsulates the features in a format suitable for the domain layer.
     ...
     * If the input features are null, an empty ModelInputs object is returned.
     */
